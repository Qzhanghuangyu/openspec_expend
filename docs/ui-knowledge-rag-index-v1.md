...
`build`、`sync`、`query`、`status`、`rebuild` 和 `clear` 均已实现。
...
当前阶段已实现安全文档装载、Markdown 分块、Fake Embedding、Manifest/chunks/vectors 原子写入以及只读 status。已实现增量同步、按文档聚合的向量查询、原子重建和派生索引清理。`build` 拒绝覆盖已有索引，Provider/模型/维度不兼容时 sync 要求 rebuild。
