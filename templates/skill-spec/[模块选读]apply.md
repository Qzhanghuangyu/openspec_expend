...
仅两个 task 技术上不可分割时可合并；开始前在 handoff 写明原因。
...
6. 读取 `operationGuidance`；只采纳不与官方状态、范围锁、Soul 和用户决定冲突的建议；
   不把 guidance 原文复制到代码、日志或 handoff。
7. 重读 tasks/comate，选择一个依赖已满足的最小未完成 task，把编号、完成条件和编辑范围
   写入 handoff 的“当前任务”。
8. 只加载当前 task 所需 context、design、规则、知识和源码证据。设计任务先复用
   父 `design.md` 已保存的设计事实和资源清单；复用本地资源前核对文件存在且哈希吻合。缺少二进制、
   必要元数据、用户告知设计变化或要求最新设计时，只重读已授权的精确
   Figma node id，无需重贴链接。
   涉及资源时按 `references/design-tools.md` 区分已有正式资源、`rawImages` 原图和节点原生 Export：
   复用正式成品无需新增透明检查；新导出透明 PNG 入库前核验 Alpha，失败不交付、不用截图或抠图补救。
   交付后按 design 台账保存项目内路径与哈希；无法安全取得时保持未完成。
   授权记录不明确、新节点或范围扩大时请求用户确认；重读有差异则回 Propose 修正 design。
9. 只实现并验证当前 task，不为后续 task 提前改动：跑通受影响方法；
   不默认新增单测，易回归纯状态逻辑优先用现有测试。普通任务轻量检查；有构建影响时，后续集成验证任务必须已在 tasks 中安排，
   handoff 标记“待集成验证”，不声称构建通过。集成节点按 `references/android-quality.md` 构建交付 variant
   的实际应用 APK；无可安装应用时验证模块。整包成功无需重复运行资源与模块任务。
   `human` 模式列人工步骤，不冒充执行；`hybrid`/`agent` 执行可完成检查。普通任务只保证自身完成条件可测试，不重复安排生命周期、资源释放或 NPE 清单。
   收尾任务一次核对最终代码的资源持有、所有权与释放；无须主动释放记“不适用”，不加回收逻辑。
   必要检查无法执行时记录缺口，返回 Propose 补独立 `[人工]` 项；未反馈前不称通过或“无泄漏”。
   UI 复现按 `references/android-quality.md` 核对 design 的关键节点文本规格、按宽适配、画布与基准宽、
   实际内容高度和 Insets；不把 `design_height_in_dp` 或设计画布高度当成实际高度，不按比例压缩整页。
   记录设备、设计/运行值及单位、差异和未取得项；必要节点无法运行核对时移入独立 `[人工]` 项，
   不绑 APK 构建。文本核对、哈希或构建不证明视觉一致。需要视觉复现验收时保持独立 `[人工]`
   项未完成，直到人工反馈页面/状态、区域、设备、预期/实际及通过/待改；不索要或
   读取页面/人工截图作视觉判断（精确节点资源异常诊断按 `references/design-tools.md`）。
   缺少人工项返回 Propose 补齐；视觉未通过则修复并重验。
