...
4. 优先复用仍有效的 preflight 证据；不得重新执行一次完整 preflight。只有证据缺失、源码变化或设计决策需要时才补充调查。
   ...
   资源的取得方式与透明验收在实际需要资源的任务中按 `references/design-tools.md` 执行；
...
5. 明确各 artifact 的职责：
   ...
   - tasks：实际任务、编辑范围、完成条件和依赖；每项任务须可验证，不默认新增单测。
     标明模块类型和交付 variant；平时安排轻量检查，完成相关实施后以一个集成任务验证构建。
     可运行入口安排实际应用 APK 构建，其成功已覆盖资源打包和代码编译；不为每个 XML/代码
     task 重复安排 Gradle。纯状态逻辑有现成测试基础时安排少量定向用例，不预排完整状态矩阵。
     对有代码改动的 change，在全部实施任务之后安排一次独立的生命周期与资源释放收尾检查；
     parallel 模式由负责代码的子 change 安排，不在每个普通任务重复核验。检查先识别实际新增或改变的
     资源持有及所有权；无需主动释放的页面记录“不适用”，不得为了通过检查添加回收代码。
     对确有风险的路径选择可执行的静态或运行时验证；必要项预知无法由 Agent 检查时另设 `[人工]`
     任务并写明步骤，执行时才发现缺口则返回 Propose 补任务。无代码改动不安排此项。
     需复现 Figma UI 时按 `references/android-quality.md` 安排关键节点运行时文本核对，并为仍需
     视觉判断的代表性页面/状态保留独立 `[人工]` 项；人工仅提供文字差异和验收结论，不上传截图。
   - comate：执行/验证模式、owner、协作状态、change 依赖和 handoff。
   先核对现有接口、状态来源和页面链路；新方案须有能力缺口证据，不把未核实的 API 行为固化为任务。
6. 页面工作先确定状态/事件与 View 契约，再按实际边界拆逻辑和可独立完成的控件，最后安排页面组装和联调。
   对当前页面需要的组件和页面模式，按 `references/ui-knowledge.md` 先筛候选并记录复用结论；
   多个可行候选且取舍影响较大时才使用轻量评分，不预设具体控件组合。
