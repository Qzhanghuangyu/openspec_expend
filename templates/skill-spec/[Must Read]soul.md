...
- 新增或实质修改的页面、组件、ViewModel、核心类和公共入口必须补充职责与边界注释。
  ...
  仅标准框架回调、无自定义语义且没有状态同步、资源操作或生命周期副作用的简单 override、
  getter/setter 和显而易见委托可以豁免；含自定义逻辑的 override 不得借此省略说明。
  状态缓存、回调抑制标记、资源所有者和非直观常量必须说明用途、单位或来源。Lambda 参数承担
  非直观业务语义时应提取为命名方法并补充参数说明，或在局部明确说明参数含义。
